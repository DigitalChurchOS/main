...
export class MockGeminiDriver implements AiCopilotProvider {
  ...
  constructor(config: { apiKey: string }) {
    ...
    if (!config.apiKey) {
      throw new Error('Invalid Gemini configuration: apiKey is required');
    }
  }

  async transcribe(audioUrl: string): Promise<AiTextResult> {
    console.log(`[MockGemini] Transcribing audio from: ${audioUrl}`);
    return {
      success: true,
  ...
  async generateSubtitles(transcript: string, format: 'srt' | 'vtt'): Promise<AiSubtitlesResult> {
    console.log(`[MockGemini] Generating ${format} subtitles for transcript`);
  ...
  async summarize(text: string): Promise<AiTextResult> {
    console.log(`[MockGemini] Summarizing text`);
  ...
  async suggestClips(text: string, options?: { numClips?: number; duration?: number; prompt?: string }): Promise<AiClipsResult> {
    ...
    console.log(`[MockGemini] Analyzing text for ${numClips} clips of ${duration}s duration${promptText}`);
  ...
  async generateBlogDraft(text: string): Promise<AiTextResult> {
    console.log(`[MockGemini] Drafting blog post from text`);
    return {
      success: true,
  ...
  async generateDevotionalDraft(text: string): Promise<AiTextResult> {
    console.log(`[MockGemini] Drafting devotional from text`);
    return {
      success: true,
  ...
  async generateSocialCaptions(text: string): Promise<AiTextResult> {
    console.log(`[MockGemini] Generating social media captions`);
    return {
      success: true,
  ...
  async translateText(text: string, targetLanguage: string): Promise<AiTextResult> {
    console.log(`[MockGemini] Translating text to ${targetLanguage}`);
    return {
      success: true,
  ...
  async generateAnnouncement(bulletPoints: string): Promise<AiTextResult> {
    console.log('[MockGemini] Generating announcement');
    return {
      success: true,
  ...
  async generateEmailDraft(topic: string, audience: string): Promise<AiTextResult> {
    console.log('[MockGemini] Generating email draft');
    return {
      success: true,
  ...
  async generateSMSDraft(text: string): Promise<AiTextResult> {
    console.log('[MockGemini] Generating SMS draft');
  ...
  async generatePushNotification(text: string): Promise<AiTextResult> {
    console.log('[MockGemini] Generating push notification');
  ...
  async generateCourseOutline(topic: string, durationWeeks: number): Promise<AiTextResult> {
    console.log('[MockGemini] Generating course outline');
  ...
  async generateQuiz(lessonContent: string, numQuestions: number): Promise<AiTextResult> {
    console.log('[MockGemini] Generating quiz');
  ...
  async generateFAQ(policyDocument: string, question: string): Promise<AiTextResult> {
    console.log('[MockGemini] Generating FAQ reply');
    return {
      success: true,
  ...
  async generateCrmReply(visitorNote: string, tone: string): Promise<AiTextResult> {
    console.log('[MockGemini] Generating CRM follow-up reply');
    return {
      success: true,
  ...
  async generateWorkflowSuggestions(churchDataJson: string): Promise<AiTextResult> {
    console.log('[MockGemini] Generating workflow suggestions');
    return {
      success: true,
