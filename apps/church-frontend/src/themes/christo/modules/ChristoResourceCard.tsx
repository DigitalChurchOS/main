...
export interface ChristoResourceCardProps {
  data: {
    id: string;
    title: string;
    description?: string;
...
export const ChristoResourceCard: React.FC<ChristoResourceCardProps> = ({ data }) => {
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        'div',
        { className: 'space-y-1' },
        React.createElement('span', { className: 'text-3xs text-[var(--christo-accent-strong)] font-bold uppercase tracking-wider' }, data.fileSize || 'PDF Document'),
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          data.cta?.label || 'Download Resource'
        )
      )
    )
  );
