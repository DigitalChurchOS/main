...
export interface ChristoMediaCardProps {
  data: {
    id: string;
    title: string;
    description?: string;
...
export const ChristoMediaCard: React.FC<ChristoMediaCardProps> = ({ data }) => {
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        'div',
        { className: 'space-y-1' },
        React.createElement('span', { className: 'text-3xs text-[var(--christo-accent-strong)] font-bold uppercase tracking-wider' }, data.type),
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          {
            variant: 'primary',
            onClick: () => { window.location.href = `/church/media/${data.id}`; },
            className: 'w-full text-center py-2'
          },
          data.cta?.label || 'Play Media'
        )
      )
    )
  );
