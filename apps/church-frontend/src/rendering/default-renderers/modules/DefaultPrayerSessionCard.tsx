...
import { PrayerSessionCardContract } from '@churchos/frontend-contracts';

export const DefaultPrayerSessionCard: React.FC<{ data: PrayerSessionCardContract }> = ({ data }) => {
  if (!data) return null;
  ...
  return React.createElement(
    ...
    React.createElement('div', { className: 'space-y-2' },
      ...
      React.createElement(
        ...
        React.createElement('div', null, `👤 Host: ${data.hostName}`),
        React.createElement('div', null, `👥 Participants: ${data.activeParticipants}`),
        React.createElement('div', null, `⏰ Start: ${new Date(data.startTime).toLocaleTimeString()}`)
      )
    ),
    React.createElement(
      'a',
