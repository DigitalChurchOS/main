...
import { ProductCardContract } from '@churchos/frontend-contracts';

export const DefaultProductCard: React.FC<{ data: ProductCardContract }> = ({ data }) => {
  if (!data) return null;
  ...
  return React.createElement(
    ...
    React.createElement('div', { className: 'space-y-2' },
      ...
      React.createElement('span', { className: 'inline-block text-xs font-bold text-slate-900 bg-slate-50 px-2 py-0.5 rounded border border-slate-100' }, `$${data.price.toFixed(2)}`)
    ),
    React.createElement(
      'a',
      {
